...
class PollEditAction extends BaseAction {
    
    public function execute() {
        if($_POST['action'] == 'edit'){
        	$poll = $this->checkAddPollData($_POST);
			$pollId = intval($_GET['pollId']);
			if($poll && $pollId){
				$this->getPollService()->updatePoll($poll,$pollId);
			}else{
				echo '更新失败';
			}
		}
	...
	public function createEditItemSetting($item){
		...
		$html .= '<dd>选项高级设置</dd>';
		$html .= '<dd>初始值：<input type="text" name="questions['.$questionId.'][items]['.$itemId.'][initCount]" value="'.$item->initCount.'" /></dd>';
    ...
    private function checkAddPollData($data){
		...
		$fields['updateUser'] = $this->getCurrentUser();
		return $fields;
    }
